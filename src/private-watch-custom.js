...
import { topupWatch as storeTopupWatch, getWatch as storeGetWatch } from 'viewkey-watch/private-watch-store';
 ...
 * shape used elsewhere in the REST plugin (one helper per family
 * of routes).
 *
 * `deps`:
 ...
 *     directly from the plugin (no circular imports, easier to test
 *     in isolation).
...
export function registerCustomTopupRoute(app, deps) {
	...
	// the plugin). Soft deps (watchDb, x402Cfg) may be null
